  ...
  return (
    <motion.button
      ...
      className={`relative p-4 rounded-2xl transition-all duration-300 group ${
        isActive ? "bg-slate-900 text-white shadow-lg" : "text-slate-600 hover:text-slate-900 hover:bg-slate-100"
      }`}
    ...
    >
      ...
      {/* Enhanced Tooltip */}
      <motion.div
        className="absolute left-full ml-6 top-1/2 transform -translate-y-1/2 bg-slate-900 text-white px-4 py-2 rounded-xl text-sm whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none shadow-xl"
        initial={{ x: -10, opacity: 0 }}
        whileHover={{ x: 0, opacity: 1 }}
      >
        {item.label}
        <motion.div
          className="absolute right-full top-1/2 transform -translate-y-1/2 border-6 border-transparent border-r-slate-900"
...
export default function FloatingNavigation() {
  ...
  return (
    <>
      ...
      <motion.button
        className="fixed top-6 right-6 z-50 md:hidden p-4 bg-white/95 backdrop-blur-sm rounded-2xl border border-slate-200 shadow-xl"
      ...
      >
        <motion.div
          className="bg-white/95 backdrop-blur-sm rounded-3xl border border-slate-200 p-4 flex flex-col space-y-4 shadow-2xl relative overflow-hidden"
          whileHover={{ scale: 1.02 }}
        >
          {/* Animated background gradient */}
          <motion.div
            className="absolute inset-0 bg-gradient-to-b from-blue-50 to-purple-50 opacity-0"
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            />
            <motion.div
              className="absolute top-20 right-6 bg-white/95 backdrop-blur-sm rounded-3xl border border-slate-200 p-6 space-y-3 shadow-2xl min-w-[200px]"
